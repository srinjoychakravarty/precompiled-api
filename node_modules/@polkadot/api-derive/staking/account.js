...
function groupByEra(list) {
  return list.reduce((map, {
    ...
    map[key] = !map[key] ? value.unwrap() : map[key].iadd(value.unwrap());
    return map;
  }, {});
}
...
function accounts(api) {
  return (0, _util2.memo)(accountIds => api.derive.session.info().pipe((0, _operators.switchMap)(sessionInfo => api.derive.staking.queryMulti(accountIds).pipe((0, _operators.map)(queries => queries.map(query => parseResult(api, sessionInfo, query)))))));
}
/**
...
function account(api) {
  return (0, _util2.memo)(accountId => api.derive.staking.accounts([accountId]).pipe((0, _operators.map)(([first]) => first)));
}